...
aa bb cc dd aaa", 2)]
        [TestCase( Puzzle, 451)]
        public void test_day_4_part_1(string passphraseDump, int expected)
        {
            int actual = GetValidPasphraseCount(passphraseDump);
...
oiii ioii iioi iiio", 3)]
        [TestCase(Puzzle, 223)]
        public void test_day_4_part_2(string passphraseDump, int expected)
        {
            int actual = GetValidPasphraseAnagramCount(passphraseDump);
            Assert.That(actual, Is.EqualTo(expected));
        }

        private int GetValidPasphraseAnagramCount(string passphraseDump)
        {
            var passphrases = passphraseDump
                .Split(new[] {"\r\n", "\r", "\n"}, StringSplitOptions.RemoveEmptyEntries)
                .Select(p =>
                    p.Split(new[] {"\t", " "}, StringSplitOptions.RemoveEmptyEntries))
                .ToList();

            var unscrambled = new List<List<string>>();
            for (int i = 0; i < passphrases.Count; i++)
            {
                var passphrase = passphrases[i].ToList();
                for (int j = 0; j < passphrase.Count; j++)
                {
                    var original = passphrase[j];
                    var ordered = string.Concat(original.OrderBy(c => c));
                    passphrase[j] = ordered;
                }
                unscrambled.Add(passphrase);
            }

            return unscrambled
                .Select(o => o.GroupBy(x => x)
                    .Select(grp => new {passhrase = grp.Key, count = grp.Count()}))
                .Count(g => g.All(x => x.count == 1));
        }

        private int GetValidPasphraseCount(string passphraseDump)
        {
            var passphrases = passphraseDump.Split(new[] { "\r\n", "\r", "\n" }, StringSplitOptions.RemoveEmptyEntries).ToList();

            return passphrases.Select(p => p.Split(new[] { "\t", " " }, StringSplitOptions.RemoveEmptyEntries)
                    .GroupBy(x => x)
                    .Select(grp => new { passhrase = grp.Key, count = grp.Count() })
                    .ToList())
                .Count(g => g.All(x => x.count == 1));

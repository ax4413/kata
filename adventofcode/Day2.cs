...
, 44216)]
        ...
        private int ChecksumPart1(string input)
        {
            var xls = GetLines(input);

            var lineChecksums = xls.Select(l => new
            {
                max = l.Max(),
                min = l.Min()
...
"
        ...
        private int ChecksumPart2(string input)
        {
            var xls = GetLines(input);
            var rt = 0;
            foreach (var line in xls)
            {
                foreach (var a in line)
                {
                    foreach (var b in line.Where(x => x != a))
                    {
                        if (a % b == 0)
                            rt += a / b;
                    }
        ...
        private static List<List<int>> GetLines(string input)
        {
